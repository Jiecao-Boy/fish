...
#need to change train_loader, test_loader

# Custom imports 
from .learner import Learner 
from utils.loss import mse, l1

# Learner to get current state and predict the action applied
# It will learn in supervised way
# 
class VisualBC(Learner):
    # Model that takes in the visual encoder/ and puts another linear layer on top?
    # then, it passes each image through the encoders,/ concats the representations ?
    ...
    def _get_repr(self, vision_image):
        if self.freeze_encoders:
            with torch.no_grad():
                vision_repr = self.image_encoder(vision_image)
        else:
    ...
    # def train_epoch(self, train_loader): ## trai_loader ? 
    def train_epoch(self, train_loader):
        self.train() 

        train_loss = 0.

        for batch in train_loader : #now batch only have image and action
            self.optimizer.zero_grad() 
            # tactile_image, vision_image, action = [b.to(self.device) for b in batch]
            vision_image, action = [b.to(self.device) for b in batch]
            # all_repr = self._get_all_repr(tactile_image, vision_image)
            repr = self._get_repr(vision_image)
            # pred_action = self.last_layer(all_repr)
            pred_action = self.last_layer(repr)
    ...
    def test_epoch(self, test_loader):
        ...
        for batch in test_loader: #now batch only have image and action
            # tactile_image, vision_image, action = [b.to(self.device) for b in batch]
            vision_image, action = [b.to(self.device) for b in batch]
            with torch.no_grad():
                # all_repr = self._get_all_repr(tactile_image, vision_image)
