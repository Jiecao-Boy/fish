...
from fish.datasets import *
from fish.environments import MockEnv
from fish.model import *
...
class Workspace:
    ...
    def train_online(self):
        ...
        while train_until_step(self.global_step): # We're going to behave as if we act and the observations and the representations are coming from the mock_demo but all the rest should be the same
            ...
            ## NOTE: If it is not the last step of an episode:
            ##       Get the action
            ## NOTE: agent involved: agent.act
            # Get the action
...
@hydra.main(version_base=None, config_path='fish/configs', config_name='train_online')
